...
export class PostService {
  ...
  getAllPosts(page: number, size: number): Observable<PostResponse[] | HttpErrorResponse> {
	...
	getPostShares(postId: number, page: number, size: number): Observable<PostResponse[]> {
		const reqParams = new HttpParams().set('page', page).set('size', size);
    return this.httpClient.get<PostResponse[]>(`${this.host}/posts/${postId}/shares`, {params: reqParams,});
	}
  ...
  getAllMyPosts(page: number, size: number): Observable<PostResponse[] | HttpErrorResponse> {
    const reqParams = new HttpParams().set('page', String(page)).set('size', String(size));
    return this.httpClient.get<PostResponse[] | HttpErrorResponse>(`${this.host}/posts/me`, {
      params: reqParams,
    });
  }
}
