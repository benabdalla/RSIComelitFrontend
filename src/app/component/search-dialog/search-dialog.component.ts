...
export class SearchDialogComponent implements OnInit, OnDestroy {
	...
	searchUser(currentPage: number): void {
		if (!this.fetchingResult) {
			const keyValue: string = String(this.key?.value || '');
			...
			if (keyValue.length >= 3) {
				...
				this.subscriptions.push(
					this.userService.getUserSearchResult(keyValue, currentPage, this.resultSize).subscribe({
							next: (resultList: UserResponse[] | HttpErrorResponse) => {
								if (!Array.isArray(resultList)) {
									// server error or unexpected shape
